...
class Train(object):
    ...
    def train(self, train_loader, val_loader):
        ...
        for epoch in range(self.args.epochs):
            ...
            self.writer.add_scalar("AUROC", auroc_score, epoch)
            self.writer.add_scalar("KAPPA", kappa_score, epoch)
            self.writer.add_scalar("top1 acc", top1, epoch)

            print("Epoch: %d, top1: %.4f, AUROC: %.4f, KAPPA: %.4f" % (epoch, top1, auroc_score, kappa_score))
            logging.info(f"Training Epoch: {epoch}\tLoss: {loss.item()}\tTop1 accuracy: {top1.item()}")
